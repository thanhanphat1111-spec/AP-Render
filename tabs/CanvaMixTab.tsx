...
import { changeMaterial, generateImage } from '../services/aiService';
import { AiServiceResult, HistoryItem, ImageFile, ObjectTransform, SourceImage } from '../types';
...
export const CanvaMixTab: React.FC<CanvaMixTabProps> = ({ onEditRequest, onVideoRequest, initialImage, onClearInitialImage }) => {
  ...
  // Helper to composite BG + Objects into a single image to send as "Image 2"
  // or as the source image if we want to treat it as an edit.
  const createInteriorComposite = async (bg: SourceImage, objects: SourceImage[], transforms: ObjectTransform[]): Promise<ImageFile | null> => {
      ...
      // Draw background
      // For "Input Interior", we often want to send just the objects overlay as Image 2
      // OR send the composite as Image 1.
      // Let's create two versions? No, let's create the composite.
      ...
      return dataURLtoFile(base64, 'composite.jpg') as any as ImageFile & { url: string, base64: string };
  };
  ...
  const createObjectsOnlyLayer = async (bg: SourceImage, objects: SourceImage[], transforms: ObjectTransform[]): Promise<ImageFile | null> => {
      ...
      // NO background draw - just objects on transparent canvas
  ...
  const handleIntGenerate = async () => {
      ...
      try {
          ...
          // Construct the strict prompt as per requirement
          // "thêm vật dụng ở ảnh 2 vào ảnh 1..."
          ...
          // Send [BG, ObjectsLayer] to generateImage
          // We assume generateImage handles multiple inputs correctly (it does in aiService)
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex flex-wrap justify-center gap-2 bg-brand-surface/50 p-2 rounded-xl backdrop-blur-sm border border-brand-primary/30">
            ...
            >
                🛋️ Input Interior (Sắp Đặt Nội Thất)
            </button>
        </div>

        {/* EXTERIOR MODE CONTENT */}
        {activeMode === 'exterior' && (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 animate-fade-in">
              <div>
                 <Card title="Đổi Vật Liệu (Exterior)">
